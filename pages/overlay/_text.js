...
import {TextField,FormLayout,Select,OptionList,ButtonGroup,Button,Toast} from '@shopify/polaris';
...
const TextOverlaySettings = (props) => {
    ...
    const {text,fontSize,fontFamily,fontcolor,bgcolor,opacity,rotation,selectedTextAlign,selectedTextPosition,selectedTextDisplayInOptions,topPadding,rightPadding,bottomPadding,leftPadding,textScaleCollection,textScaleProduct,textScaleSearch,overlayEditId,displayFontColorPicker,displayBgColorPicker} = props.stateKeys;
	const {setText,setFontSize,setFontFamily,setFontcolor,setBgcolor,setOpacity,setRotation,setSelectedTextAlign,setselectedTextPosition,setTextDisplayInOptions,setTopPadding,setRightPadding,setBottomPadding,setLeftPadding,setTextScaleCollection,setTextScaleProduct,setTextScaleSearch,setOverlayEditId,setDisplayFontColorPicker,setDisplayBgColorPicker} = props.stateFunc;
    //#endregion
    ...
    const [addEditOverlay, addEditOverlayRes] = useMutation(MutationShopify.SAVE_OVERLAY_SETTINGS(),{client:props.restClient});
    const handleTextOverlay = async () => {
        const overlayData = {
            ...
            'status':'Active',
            'display_in_collection': _.includes(selectedTextDisplayInOptions, 'COLLECTION') ? 'Yes' : 'No',
            'display_in_product':_.includes(selectedTextDisplayInOptions, 'PRODUCT') ? 'Yes' : 'No',
            'display_in_search':_.includes(selectedTextDisplayInOptions, 'IN_SEARCH') ? 'Yes' : 'No',
            'scale_in_collection':textScaleCollection,
            'scale_in_product':textScaleProduct,
            'scale_in_search':textScaleSearch,
            'overlay_id':(overlayEditId != null && overlayEditId > 0)?overlayEditId:'',  
        };
        const { data, errors} = await addEditOverlay({variables:{overlayData}});
        if(!errors && data.addEditOverlay){            
            setQueryToast([<Toast onDismiss={() => {
                setQueryToast([]);
            }} duration="1500" content={data.addEditOverlay?.message} error={Number(data.addEditOverlay.code) !== 200 ? true : false} />]); 
        }
        props.refreshData();
        setTimeout(function(){ 
            location.reload();
                ...
                <FormLayout>                
                    ...
                    <FormLayout.Group>
                        <div className="text_overlay_font_color">
                            <TextField 
                                ...
                                connectedRight = {
                                    <button style={{background:fontcolor}} onClick={()=>{setDisplayBgColorPicker(false); displayFontColorPicker ? setDisplayFontColorPicker(false) : setDisplayFontColorPicker(true)}} className="Polaris-Button font_color_connector">
                                    </button>
                                }
                            />
                            { displayFontColorPicker ? 
                                <div className="display_font_color font_color_connector">
                                    <SketchPicker 
                                        color={fontcolor}
                                        onChangeComplete={(value)=>handleChangeFontColorComplete(value)}
                                    />
                                </div> : null
                            }
                        </div>
                        <div className="text_overlay_bg_color">
                            <TextField 
                                ...
                                connectedRight = {
                                    <button style={{background:bgcolor}} onClick={()=>{ setDisplayFontColorPicker(false); displayBgColorPicker ? setDisplayBgColorPicker(false) : setDisplayBgColorPicker(true)}} className="Polaris-Button bg_color_connector"></button>
                                }
                            />
                            { displayBgColorPicker ? 
                                <div className="display_background_color bg_color_connector">
                                    <SketchPicker 
                                        color={bgcolor}
                                        onChangeComplete={(value)=>handleChangeBgColorComplete(value)}
                                    />
                                </div> : null
                            }
                        </div>
                    ...
                    <FormLayout.Group>
                        ...
                        <Select
                            ...
                            onChange={(value) => {setSelectedTextAlign(value)}}
                        />
                        <Select
                            label="Text position"
                            options={textPosition}
                            value={selectedTextPosition}
                            onChange={(value) => {setselectedTextPosition(value)}}
                        />
                    </FormLayout.Group>
                    <FormLayout.Group>
                        <TextField 
                    ...
                    <div className="overlay_action_btn">
                        <FormLayout.Group>
                            <ButtonGroup spacing="loose">
                                <Button loading={isLoading} onClick={()=>{setIsLoading(true);location.reload();}}>Cancel</Button>
                                <Button primary loading={addEditOverlayRes.loading} onClick={() => handleTextOverlay()}>Save</Button>
                            </ButtonGroup>
                        </FormLayout.Group>
                    </div>
                </FormLayout>
