...
import {Stack,DropZone, Thumbnail,FormLayout,Select, Banner, List,OptionList,ButtonGroup,Button,TextField,ChoiceList,Pagination,RangeSlider,Toast} from '@shopify/polaris';
...
const ImageOverlaySettings = (props) => {  
    ...
    const fileUpload = !file && <DropZone.FileUpload actionTitle="Add image" actionHint="or drop image to upload" />;
    ...
    const chooseBadgeUrl = (badge_id,badge_url) =>{
        ...
        var selectImgElem = document.getElementsByClassName('badge_img_'+badge_id)[0];
        if (selectImgElem) {
    ...
    const handleImageOverlay = async () => {
        const overlayData = {
            ...
            'display_in_search':_.includes(selectedImageDisplayInOptions, 'IN_SEARCH') ? 'Yes' : 'No',
        ...
        if(!errors && data.addEditOverlay){            
            setQueryToast([<Toast key="toast-msg" onDismiss={() => {
                setQueryToast([]);
        ...
        <React.Fragment>
            ...
            <div className="image_overlay_settings_container">
                <FormLayout>
                    ...
                    :
                        <Query query={ QueryRootShopify.GET_BADGES()} client={props.restClient} variables={{
                            "limit":limit,
                            "after":afterCursor, 
                            "before":beforeCursor
                        }}>
                            {({ data, loading, error, refetch}) => {
                                ...
                                if(!loading){
                                    ...
                                    for (let i = 0; i < data.badges.data.edges.length; i++) {
                                        ...
                                        imgThumbnail.push(
                                            <div onClick={()=>chooseBadgeUrl(bdg.id, bdg.badge_url)} className={"badge_img_section badge_img_"+bdg.id} key={`gallery-img--container${i}`}>
                                                <Thumbnail
                                                    key={`gallery-img-${i}`}
                    ...
                    <div className="overlay_action_btn">
                        <FormLayout.Group>
                            <ButtonGroup spacing="loose">    
                                ...
                                <Button primary loading={addEditOverlayRes.loading} onClick={() => handleImageOverlay()}>Save</Button>
                            </ButtonGroup>
                        </FormLayout.Group>
                    </div>
                </FormLayout>
