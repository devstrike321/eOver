...
const PlanIndex = (props) => {
      ...
      for (let i = 0; i < planData?.plans?.data?.length; i++) {
        ...
        if (planData.plans.data[i].plan_monthly_price != 0) {
          if (planData.plans.data[i].plan_activated) {
            ...
            if (planData.plans.data[i].access_products == "UNLIMITED") {
              isBtnDisable = false;
            } else if (
              shopProductOverlayCount <= planData.plans.data[i].access_products
            ) {
              isBtnDisable = false;
            }
            button = (
              <React.Fragment>
                <Button
                  ...
                  onClick={() =>
                    upgradePlan(
                      planData.plans.data[i].id,
                      planData.plans.data[i].plan_title,
                      planData.plans.data[i].plan_monthly_price,
                      planData.plans.data[i].access_products
                    )
                  }
                >
                  {planData.plans.data[i].access_products != "UNLIMITED"
                    ? "Downgrade"
                    : "Upgrade"}
                </Button>
              </React.Fragment>
            ...
            <Layout.Section oneThird>
              <Card sectioned>
                <Stack alignment="center" vertical={true} distribution="fill">
                  ...
                  <DisplayText>
                    {planData.plans.data[i].plan_monthly_price == 0
                      ? `${planData.plans.data[i].pm_trial_days} days free trial`
                      : planData.plans.data[i].access_products + " Products"}
                  </DisplayText>
                  <DisplayText>
                    {"$" + planData.plans.data[i].plan_monthly_price}
                  </DisplayText>
  ...
  const [selectPlan, selectPlanRes] = useMutation(
    MutationShopify.SELECT_PLAN(),
    { client: props.restClient }
  );
  const selectNewPlan = async (plan_id, plan_title, plan_monthly_price) => {
    ...
    const { data, errors } = await selectPlan({
      variables: { selectPlanInfo },
    });

    if (parseInt(plan_monthly_price) == 0) {
      let freePlanRedirectURl =
        process.env.NEXT_PUBLIC_HOST +
        process.env.NEXT_PUBLIC_FOLDER_PATH +
        `get-shopify-plan?shop_name=${shop}&sel_plan=${plan_id}&charge_id=`;
      redirect.dispatch(Redirect.Action.REMOTE, freePlanRedirectURl);
      return true;
    }

    if (!errors && data.selectPlan) {
      let confirmation_url = await createNewSubscription(
        plan_id,
        plan_title,
        plan_monthly_price
      );
  ...
  const [createSubscription, createSubscriptionRes] = useMutation(
    MutationShopify.CREATE_SUBSCRIPTION_IN_SHOPIFY()
  );
  const createNewSubscription = async (
    ...
    const returnUrl =
      process.env.NEXT_PUBLIC_HOST +
      process.env.NEXT_PUBLIC_FOLDER_PATH +
      `get-shopify-plan?shop_name=${shop}&sel_plan=${plan_id}`;
    ...
    if (!errors && data.appSubscriptionCreate) {
      let charge_id = data?.appSubscriptionCreate?.appSubscription?.id.replace(
        "gid://shopify/AppSubscription/",
        ""
      );
      let confirmation_url = data?.appSubscriptionCreate?.confirmationUrl;

      let isSubscriptionUpdated = await updateSubscriptionInfo(
        plan_id,
        charge_id,
        confirmation_url,
        returnUrl
  ...
  const updateSubscriptionInfo = async (
    ...
    const { data, errors } = await updateSubscription({
      variables: { subscriptionInfo, id: plan_id },
    });
        ...
        {chargeBanner}
        {
          <Modal
            ...
            primaryAction={{
              ...
              onAction: () =>
                selectNewPlan(
                  chooseNewPlanInfo.id,
                  chooseNewPlanInfo.plan_title,
                  chooseNewPlanInfo.plan_monthly_price
                ),
            ...
            <Modal.Section>
              <TextContainer>
                {chooseNewPlanInfo.access_products == "UNLIMITED" ? (
                  <p>
                    You are going to choose {chooseNewPlanInfo.plan_title}. Here
                    you can set overlay in {chooseNewPlanInfo.access_products}{" "}
                    products.
                  </p>
                ) : (
                  <p>
                    You are going to choose {chooseNewPlanInfo.plan_title}. Here
                    you can set overlay in max{" "}
                    {chooseNewPlanInfo.access_products} products. If you have
                    overlay in more than {chooseNewPlanInfo.access_products}{" "}
                    products, then it will be removed.This can’t be undone
                  </p>
                )}
              </TextContainer>
            </Modal.Section>
